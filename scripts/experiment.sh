    ...
    for br in "${BITRATES[@]}"; do
        for file in $RECS_PATH/*.yuv; do
            for (( i=0; i<$REPEATS; i++ )); do   
                ...
                params="--quiet --no-progress --profile baseline --vbv-bufsize $((4*$br)) --bitrate $br --threads 1 --tune zerolatency --fps $FPS --input-res $RESOLUTION -o "$RES_PATH"/$out_file.h264 $file"
                if [ "$mode" != "none" ]; then
